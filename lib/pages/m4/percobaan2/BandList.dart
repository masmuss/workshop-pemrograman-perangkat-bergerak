...
import 'package:test/pages/m4/percobaan2/DetailScreen.dart';
import 'package:test/pages/m4/percobaan2/ListItem.dart';
    ...
    return ListView.builder(
      itemBuilder: (context, index) {
        ...
        return InkWell(
          ...
          child: Consumer<DoneListeningProvider>(
            builder: (context, DoneListeningProvider data, widget) {
              return ListItem(
                band: band,
                isDone: data.doneListening.contains(band),
                onCheckboxClick: (bool? value) {
                  setState(() {
                    if (value != null) {
                      value
                          ? data.doneListening.add(band)
                          : data.doneListening.remove(band);
                    }
                  });
                },
              );
